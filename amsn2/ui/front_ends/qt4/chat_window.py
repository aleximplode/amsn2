...
class aMSNChatWidget(QtGui.QWidget, base.aMSNChatWidget):
    def __init__(self, amsn_conversation, parent, contacts_uid):
        ...
        remoteContactMessage = (remoteContact.personal_message.parse_default_smileys().to_HTML_string() if (len(str(remoteContact.current_media.parse_default_smileys().to_HTML_string())) == 0) else remoteContact.current_media.parse_default_smileys().to_HTML_string())
        remoteContactStatus = remoteContact.status.parse_default_smileys().to_HTML_string()
        
        self.ui.userinfo.setTextFormat(1) #rich text
        self.ui.userinfo.setText("To: {0} &lt;{1}&gt; {2} ({3})".format(remoteContactNick, remoteContact.account, remoteContactMessage, remoteContactStatus))
